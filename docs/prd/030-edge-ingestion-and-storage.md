...
| F-030-41 | Retention prunes all transport markers older than 7 d. Durable effects have independent stable uniqueness/order; `device.status` uses a bounded per-device boot-generation/sequence/LWT high-water projection, so retained/LWT redelivery cannot refresh state after marker pruning. Synced outbox rows older than 24 h and quarantine beyond 1000 rows are pruned. |
