...
M12's first issue (`M12-001`) depends on `M6-019`, so the UI becomes buildable
...
```text
M0-013 + M1-014 ──→ M3-001 (edge binary + supervisor)
...
```text
M3-016 ──→ M4-001 (status ingestion)
   ├──→ M4-002 (LWT)
...
```text
M4-011 ──→ M5-001 (plant/profile repositories)
...
```text
M5-013 ──→ M6-001 (IrrigationInputs / IrrigationDecision)
              ▼
...
```text
M6-019 ──→ M7-001 (cloud binary + postgres) ──→ M7-002 (schema) ──→ M7-003 (ingestion)
                                                                        ├──→ M7-004
                                                                        │  (projections)
...
```text
M8-015 ──→ M9-001 (verify + correct ADR-007 toolchain)   ◄── do this first, on real hardware
   ├──→ M9-002 (firmware CI job)
   └──→ M9-003 (workspace skeleton)
           ├──→ M9-004 (NVS + identity) ──┬──→ M9-008 (wifi + sntp) ──→ M9-009 (mqtt + lwt)
...
```text
M9-015 ──→ M10-001 (adapter selection)
...
```text
M6-019 ──→ M12-001 (Tauri + Leptos workspace, no Node)
              ▼
...
```text
M12-013 ──→ M13-001 (multi-device operation + cross-plant isolation)
   ...
   M13-011 + M12-013 ──→ M13-012 (UI at scale)
                   all ──→ M13-013 (verification)
```

### M14 — Field readiness (documentation only)

```text
M13-013 ──→ M14-001 (verify reservations against code)
...
| M7-014 | M6-019 | emits events from every M6 state change |
| M10-008 | M6-013 | extends the config publication path |
| M12-001 | M6-019 | needs state and actions worth showing |
| M13-012 | M12-013 | extends the completed UI |
...
M0-001 → M0-002 → M0-003 → M0-012 → M0-013
       → M1-001 → M1-004 → M1-008 → M1-009 → M1-014
       → M2-001 → M2-002 → M2-003 → M2-006 → M2-008 → M2-015
       → M3-001 → M3-002 → M3-003 → M3-008 → M3-009 → M3-016
       → M4-001 → M4-008 → M4-011
       → M5-001 → M5-005 → M5-009 → M5-012 → M5-013
       → M6-001 → M6-002 → M6-004 → M6-005 → M6-006 → M6-007
                → M6-008 → M6-009 → M6-010 → M6-012 → M6-018 → M6-019
       → M7-001 → M7-002 → M7-003 → M7-005 → M7-006 → M7-015
       → M8-001 → M8-002 → M8-003 → M8-004 → M8-005 → M8-006 → M8-012 → M8-015
```
