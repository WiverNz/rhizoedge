...
async fn tick(
    ...
    let rows = sqlx::query("SELECT device_id,status,last_seen_at,telemetry_interval_seconds,desired_config_version,applied_config_version,drift_since,connectivity_mode,last_time_sync_at FROM devices")
    ...
    for row in rows {
        ...
        let stale = status == "online"
            && last_seen.is_some_and(|seen| {
                sample_age_seconds(now, seen) >= stale_after_seconds(telemetry)
            });
        if stale {
        ...
        if row.get::<String, _>("connectivity_mode") == "isolated" && status == "online" {
            isolated += 1;
        }
