...
pub async fn run_batch(db: &EdgeDb, now: i64, batch: u32) -> Result<Pruned, StorageError> {
    let processed=del(db,"DELETE FROM processed_messages WHERE message_id IN (SELECT message_id FROM processed_messages WHERE received_at < ? ORDER BY received_at LIMIT ?)",now-7*86_400_000,batch).await?;
